...
class LSTMNetMamba(nn.Module):
    """
    Mamba+LSTM Network based on LSTMNetVIT architecture
    Replaces transformer encoders with Mamba SSM encoders
    """
    def __init__(self):
        super().__init__()
        self.encoder_blocks = nn.ModuleList([
            MambaEncoderLayer(1, 32, patch_size=7, stride=4, padding=3, n_layers=2, d_state=16, d_conv=4, expand=2),
            MambaEncoderLayer(32, 64, patch_size=3, stride=2, padding=1, n_layers=2, d_state=16, d_conv=4, expand=2)
        ])
